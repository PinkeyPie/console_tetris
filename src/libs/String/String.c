...
int Alloc(String string, int length) {
    int len;
    if (string == NULL || string->data == NULL || string->slen < 0 || string->mlen <= 0 ||
        string->mlen < string->slen || length <= 0) {
        return STR_ERR;
    }

    if (length >= string->mlen) {
        unsigned char* x;
        if ((len = SnapUpSize(length)) <= string->mlen) {
            return STR_OK;
        }
        // Assume probability of a non-moving realloc is 0.125
        if (7 * string->mlen < 8 * string->slen) {
            ...
            if (NULL == (x = (unsigned char*)malloc((size_t)len))) {
                ...
                if (string->slen) {
                    memcpy((char*)x, (char*)string->data, (size_t)string->slen);
                }
                free(string->data);
            }
        }
        string->data = x;
        string->mlen = len;
        string->data[string->slen] = (unsigned char)'\0';
    }
...
int AllocMin(String string, int length) {
    ...
    if (string->slen >= INT_MAX || string->slen < 0) return STR_ERR;
    if (string->mlen <= 0 || string->mlen < string->slen || length <= 0) {
        return STR_ERR;
    }

    if (length < string->slen + 1) length = string->slen + 1;

    if (length != string->mlen) {
        s = (unsigned char*)realloc(string->data, (size_t)length);
        if (NULL == s) return STR_ERR;
        s[string->slen] = (unsigned char)'\0';
        string->data = s;
        string->mlen = length;
    }
 ...
 *  buffer str.
 */
String FromCStr(const char* str) {
    String b;
    int i;
    size_t j;
    ...
    b = (String)malloc(sizeof(struct _TagString));
 ...
 *  maxl characters.
 */
String FromCStrRangeAlloc(int minl, int maxl, const char* str) {
    String b;
    int i;
    size_t j;
    ...
    while (NULL == (b->data = (unsigned char*)malloc(b->mlen = i))) {
        int k = (i >> 1) + (minl >> 1);
        if (i == k || i < minl) {
            malloc(b);
            return NULL;
 ...
 */
String FromCStrAlloc(int mlen, const char* str) {
    return FromCStrRangeAlloc(mlen, mlen, str);
}
 ...
 *  len.
 */
String BytesToString(const void* blk, int len) {
    String b;
    int i;
...
/*  char * bstr2cstr (const_bstring s, char ch)
 *
...
char* StringToCStr(ConstString string, char ch) {
    ...
    if (string == NULL || string->slen < 0 || string->data == NULL) return NULL;
    l = string->slen;
 ...
 */
int Concat(String b0, ConstString b1) {
    int len, d;
    String aux = (String)b1;
    ...
    d = b0->slen;
    len = b1->slen;
    if ((d | (b0->mlen - d) | len | (d + len)) < 0) return STR_ERR;

    if (b0->mlen <= d + len + 1) {
        ptrdiff_t pd = b1->data - b0->data;
        if (0 <= pd && pd < b0->mlen) {
            if (NULL == (aux = StrCpy(b1))) return STR_ERR;
        }
        if (Alloc(b0, d + len + 1) != STR_OK) {
            if (aux != b1) Destroy(aux);
            return STR_ERR;
        }
    }
 ...
 */
int ConcatCh(String b, char c) {
    int d;

    if (b == NULL) return STR_ERR;
    d = b->slen;
    if ((d | (b->mlen - d)) < 0 || Alloc(b, d + 2) != STR_OK)
        return STR_ERR;
 ...
 */
int ConcatCStr(String b, const char* s) {
    char* d;
    int i, l;

    if (b == NULL || b->data == NULL || b->slen < 0 || b->mlen < b->slen
        || b->mlen <= 0 || s == NULL) return STR_ERR;
    ...
    return ConcatBytes(b, (const void*)s, (int)strlen(s));
}
 ...
 */
int ConcatBytes(String b, const void* s, int len) {
    int nl;

    if (b == NULL || b->data == NULL || b->slen < 0 || b->mlen < b->slen
        || b->mlen <= 0 || s == NULL || len < 0) return STR_ERR;

    if (0 > (nl = b->slen + len)) return STR_ERR; /* Overflow? */
    if (b->mlen <= nl && 0 > Alloc(b, nl + 1)) return STR_ERR;
...
String StrCpy(ConstString b) {
    ...
    b0 = (String)StrAlloc(sizeof(struct _TagString));
    if (b0 == NULL) {
    ...
    b0->data = (unsigned char*)StrAlloc(j);
    if (b0->data == NULL) {
        j = i + 1;
        b0->data = (unsigned char*)StrAlloc(j);
        if (b0->data == NULL) {
    ...
    if (i) StrMemcpy((char*)b0->data, (char*)b->data, i);
 ...
 */
int Assign(String a, ConstString b) {
    if (b == NULL || b->data == NULL || b->slen < 0)
        return STR_ERR;
    if (b->slen != 0) {
        if (Alloc(a, b->slen) != STR_OK) return STR_ERR;
 ...
 */
int AssignMidStr(String a, ConstString b, int left, int len) {
    if (b == NULL || b->data == NULL || b->slen < 0)
        return STR_ERR;
    ...
    if (len > 0) {
        if (Alloc(a, len) != STR_OK) return STR_ERR;
 ...
 */
int AssignCStr(String a, const char* str) {
    int i;
    size_t len;
    if (a == NULL || a->data == NULL || a->mlen < a->slen ||
    ...
    if (len + 1 > (size_t) INT_MAX - i ||
        0 > Alloc(a, (int)(i + len + 1))) return STR_ERR;
 ...
 */
int AssignBytes(String a, const void* s, int len) {
    if (a == NULL || a->data == NULL || a->mlen < a->slen ||
        a->slen < 0 || a->mlen == 0 || NULL == s || len < 0 || len >= INT_MAX)
        return STR_ERR;
    if (len + 1 > a->mlen && 0 > Alloc(a, len + 1)) return STR_ERR;
 ...
 */
int Trunc(String b, int n) {
    if (n < 0 || b == NULL || b->data == NULL || b->mlen < b->slen ||
        b->slen < 0 || b->mlen <= 0) return STR_ERR;
    if (b->slen > n) {
        b->slen = n;
 ...
 */
int StrComp(ConstString b0, ConstString b1) {
    int i, v, n;

    if (Data(b0) == NULL || b0->slen < 0 ||
        Data(b1) == NULL || b1->slen < 0) return SHRT_MIN;
    if ((n = b0->slen) > b1->slen) n = b1->slen;
    else if (b0->slen == b1->slen && b0->data == b1->data) return STR_OK;
 ...
 *  UCHAR_MAX+1.
 */
int StrnComp(ConstString b0, ConstString b1, int n) {
    int i, v, m;

    if (Data(b0) == NULL || b0->slen < 0 ||
        Data(b1) == NULL || b1->slen < 0 || n < 0) return SHRT_MIN;
    m = n;
    ...
    if (n == m || b0->slen == b1->slen) return STR_OK;
 ...
 */
int EqualsCaseLessBlk(ConstString b, const void* blk, int len) {
    int i;

    if (Data(b) == NULL || b->slen < 0 ||
        blk == NULL || len < 0) return STR_ERR;
 ...
 */
int EqualsCaseLess(ConstString b0, ConstString b1) {
    if (NULL == b1) return STR_ERR;
    return EqualsCaseLessBlk(b0, b1->data, b1->slen);
}
 ...
 *  way.
 */
int StemEqualsCaseLessBlk(ConstString b0, const void* blk, int len) {
    int i;

    if (Data(b0) == NULL || b0->slen < 0 || NULL == blk || len < 0)
        return STR_ERR;
    if (b0->slen < len) return STR_OK;
 ...
 *  special way.
 */
int EqualsBytes(ConstString b, const void* blk, int len) {
 ...
 */
int Equals(ConstString b0, ConstString b1) {
    if (b0 == NULL || b1 == NULL || b0->data == NULL || b1->data == NULL ||
        b0->slen < 0 || b1->slen < 0) return STR_ERR;
    if (b0->slen != b1->slen) return STR_OK;
 ...
 *  any special way.
 */
int StemEqualsBytes(ConstString b0, const void* blk, int len) {
    int i;
    ...
    for (i = 0; i < len; i++) {
        if (b0->data[i] != ((const unsigned char*)blk)[i]) return STR_OK;
    }
    return 1;
}
 ...
 */
int EqualsCStr(ConstString b, const char* s) {
    int i;
 ...
 */
int EqualsCStringCaseLess(ConstString b, const char* s) {
    int i;
    if (b == NULL || s == NULL || b->data == NULL || b->slen < 0)
        return STR_ERR;
    for (i = 0; i < b->slen; i++) {
 ...
 */
int bstrcmp(ConstString b0, ConstString b1) {
    int i, v, n;
    ...
    for (i = 0; i < n; i++) {
        ...
        if (v != 0) return v;
        if (b0->data[i] == (unsigned char)'\0') return STR_OK;
    }
 ...
 */
int bstrncmp(ConstString b0, ConstString b1, int n) {
    int i, v, m;
 ...
 */
String Substring(ConstString b, int left, int len) {
    ...
    if (len <= 0) return FromCStr("");
    return BytesToString(b->data + left, len);
}
 ...
 */
int Remove(String b, int pos, int len) {
 ...
 */
int Contains(ConstString b1, int pos, ConstString b2) {
    int j, ii, ll, lf;
    unsigned char* d0;
    unsigned char c0;
    ...
    if (b1 == NULL || b1->data == NULL || b1->slen < 0 ||
        b2 == NULL || b2->data == NULL || b2->slen < 0) return STR_ERR;
    if (b1->slen == pos) return (b2->slen == 0) ? pos : STR_ERR;
    if (b1->slen < pos || pos < 0) return STR_ERR;
    if (b2->slen == 0) return pos;

    /* No space to find such a string? */
    if ((lf = b1->slen - b2->slen + 1) <= pos) return STR_ERR;
 ...
 *  this can take much longer than it needs to.
 */
int ContainsReverse(ConstString b1, int pos, ConstString b2) {
    int j, i, l;
    unsigned char* d0, * d1;
    ...
    i = pos;
    if ((l = b1->slen - b2->slen) < 0) return STR_ERR;
 ...
 */
int ContainsCaseLess(ConstString b1, int pos, ConstString b2) {
    int j, i, l, ll;
    unsigned char* d0, * d1;
    ...
    /* No space to find such a string? */
    if (l <= pos) return STR_ERR;

    /* An obvious alias case */
    if (b1->data == b2->data && pos == 0) return STR_OK;
 ...
 *  many degenerate cases where this can take much longer than it needs to.
 */
int binstrrcaseless(ConstString b1, int pos, ConstString b2) {
    int j, i, l;
    unsigned char* d0, * d1;
 ...
 *  (inclusive).
 */
int ContainsCh(ConstString b, int c, int pos) {
    unsigned char* p;
 ...
 *  (inclusive).
 */
int ContainsChReverse(ConstString b, int c, int pos) {
    int i;
...
static int buildCharField(struct charField* cf, ConstString b) {
    int i;
    if (b == NULL || b->data == NULL || b->slen <= 0) return STR_ERR;
 ...
 */
int Search(ConstString b0, int pos, ConstString b1) {
    struct charField chrs;
    if (pos < 0 || b0 == NULL || b0->data == NULL ||
        b0->slen <= pos) return STR_ERR;
    if (1 == b1->slen) return ContainsCh(b0, b1->data[0], pos);
    if (0 > buildCharField(&chrs, b1)) return STR_ERR;
 ...
 */
int SearchReverse(ConstString b0, int pos, ConstString b1) {
    struct charField chrs;
    if (pos < 0 || b0 == NULL || b0->data == NULL || b1 == NULL ||
        b0->slen < pos) return STR_ERR;
    if (pos == b0->slen) pos--;
    if (1 == b1->slen) return ContainsChReverse(b0, b1->data[0], pos);
    if (0 > buildCharField(&chrs, b1)) return STR_ERR;
 ...
 *  does not exist in b0, then BSTR_ERR is returned.
 */
int SearchExclude(ConstString b0, int pos, ConstString b1) {
    struct charField chrs;
    if (pos < 0 || b0 == NULL || b0->data == NULL ||
        b0->slen <= pos) return STR_ERR;
    if (buildCharField(&chrs, b1) < 0) return STR_ERR;
 ...
 *  exist in b0, then BSTR_ERR is returned.
 */
int SearchExcludeReverse(ConstString b0, int pos, ConstString b1) {
    struct charField chrs;
    if (pos < 0 || b0 == NULL || b0->data == NULL ||
        b0->slen < pos) return STR_ERR;
    if (pos == b0->slen) pos--;
 ...
 */
int SetString(String b0, int pos, ConstString b1, unsigned char fill) {
    int d, newlen;
    ptrdiff_t pd;
    String aux = (String)b1;

    if (pos < 0 || b0 == NULL || b0->slen < 0 || NULL == b0->data ||
        b0->mlen < b0->slen || b0->mlen <= 0) return STR_ERR;
    if (b1 != NULL && (b1->slen < 0 || b1->data == NULL)) return STR_ERR;
 ...
 */
int InsertBytes(String b, int pos, const void* blk, int len,
                unsigned char fill) {
    int d, l;
    unsigned char* aux = (unsigned char*)blk;

    if (b == NULL || blk == NULL || pos < 0 || len < 0 || b->slen < 0 ||
        b->mlen <= 0 || b->mlen < b->slen) return STR_ERR;

    /* Compute the two possible end pointers */
    d = b->slen + len;
    l = pos + len;
    if ((d | l) < 0) return STR_ERR; /* Integer wrap around. */

    /* Aliasing case */
    if (((size_t)((unsigned char*)blk + len)) >= ((size_t)b->data) &&
        ((size_t)blk) < ((size_t)(b->data + b->mlen))) {
        if (NULL == (aux = (unsigned char*)StrAlloc(len)))
            return STR_ERR;
        StrMemcpy(aux, blk, len);
    }

    if (l > d) {
        /* Inserting past the end of the string */
        if (Alloc(b, l + 1) != STR_OK) {
            if (aux != (unsigned char*)blk) StrFree(aux);
            return STR_ERR;
        }
 ...
 */
int Insert(String b1, int pos, ConstString b2, unsigned char fill) {
    if (NULL == b2 || (b2->mlen > 0 && b2->slen > b2->mlen)) return STR_ERR;
    return InsertBytes(b1, pos, b2->data, b2->slen, fill);
}
 ...
 */
int Replace(String b1, int pos, int len, ConstString b2,
            unsigned char fill) {
    int pl, ret;
    ptrdiff_t pd;
    String aux = (String)b2;

    if (pos < 0 || len < 0) return STR_ERR;
    if (pos > INT_MAX - len) return STR_ERR; /* Overflow */
    pl = pos + len;
    if (b1 == NULL || b2 == NULL || b1->data == NULL || b2->data == NULL ||
        b1->slen < 0 || b2->slen < 0 || b1->mlen < b1->slen ||
        b1->mlen <= 0) return STR_ERR;

    /* Straddles the end? */
    if (pl >= b1->slen) {
        if ((ret = SetString(b1, pos, b2, fill)) < 0) return ret;
    ...
    if ((pd = (ptrdiff_t)(b2->data - b1->data)) >= 0 &&
        ...
        if (NULL == (aux = StrCpy(b2))) return STR_ERR;
    }

    if (aux->slen > len) {
        if (Alloc(b1, b1->slen + aux->slen - len) != STR_OK) {
            if (aux != b2) Destroy(aux);
            return STR_ERR;
        }
    }

    if (aux->slen != len) memmove (b1->data + pos + aux->slen,
                                   b1->data + pos + len,
                                   b1->slen - (pos + len));
    StrMemcpy(b1->data + pos, aux->data, aux->slen);
...
typedef int (*instr_fnptr) (ConstString s1, int pos, ConstString s2);

#define INITIAL_STATIC_FIND_INDEX_COUNT 32

static int findreplaceengine(String b, ConstString find,
    ...
    if (delta == 0) {
        while ((pos = instr(b, pos, auxf)) >= 0) {
            StrMemcpy(b->data + pos, auxr->data, auxr->slen);
            pos += auxf->slen;
        }
        if (auxf != find) Destroy(auxf);
        if (auxr != repl) Destroy(auxr);
        return STR_OK;
    }
    ...
    if (delta > 0) {
        ...
        while ((i = instr(b, pos, auxf)) >= 0) {
            ...
            if (auxr->slen)
                StrMemcpy(b->data + i - acc, auxr->data, auxr->slen);
            acc += delta;
            pos = i + auxf->slen;
        }
    ...
    */
    ...
    while ((pos = instr(b, pos, auxf)) >= 0) {
        if (slen >= mlen - 1) {
            ...
            if (NULL == d) StrMemcpy(t, static_d, sizeof(static_d));
            d = t;
        }
        d[slen] = pos;
        slen++;
    ...
    if (STR_OK == (ret = Alloc(b, b->slen + acc + 1))) {
        b->slen += acc;
        for (i = slen - 1; i >= 0; i--) {
            int s, l;
            s = d[i] + auxf->slen;
 ...
 */
int FindReplace(String b, ConstString find, ConstString repl,
                int pos) {
    return findreplaceengine(b, find, repl, pos, Contains);
}
 ...
 */
int FindReplaceCaseLess(String b, ConstString find, ConstString repl,
                        int pos) {
    return findreplaceengine(b, find, repl, pos, ContainsCaseLess);
}
 ...
 */
int InsertCh(String b, int pos, int len, unsigned char fill) {
    int d, l, i;

    if (pos < 0 || b == NULL || b->slen < 0 || b->mlen < b->slen ||
        b->mlen <= 0 || len < 0) return STR_ERR;

    /* Compute the two possible end pointers */
    d = b->slen + len;
    l = pos + len;
    if ((d | l) < 0) return STR_ERR;

    if (l > d) {
        /* Inserting past the end of the string */
        if (Alloc(b, l + 1) != STR_OK) return STR_ERR;
        pos = b->slen;
        b->slen = l;
    }
    else {
        /* Inserting in the middle of the string */
        if (Alloc(b, d + 1) != STR_OK) return STR_ERR;
        for (i = d - 1; i >= l; i--) {
 ...
 */
int Pattern(String b, int len) {
    int i, d;

    d = Length(b);
    if (d <= 0 || len < 0 || Alloc(b, len + 1) != STR_OK) return STR_ERR;
    if (len > 0) {
        if (d == 1) return SetString(b, len, NULL, b->data[0]);
 ...
 *  efficient way.
 */
int ReadA(String b, bNread readPtr, void* parm) {
    int i, l, n;

    if (b == NULL || b->mlen <= 0 || b->slen < 0 || b->mlen < b->slen ||
        readPtr == NULL) return STR_ERR;

    i = b->slen;
    for (n = i + 16; ; n += ((n < BS_BUFF_SZ) ? n : BS_BUFF_SZ)) {
        if (STR_OK != Alloc(b, n + 1)) return STR_ERR;
 ...
 *  efficient way.
 */
String Read(bNread readPtr, void* parm) {
    String buff;

    if (0 > ReadA(buff = FromCStr(""), readPtr, parm)) {
        Destroy(buff);
        return NULL;
    }
    return buff;
 ...
 */
int AssignGetStr(String b, bNgetc getcPtr, void* parm, char terminator) {
    int c, d, e;

    if (b == NULL || b->mlen <= 0 || b->slen < 0 || b->mlen < b->slen ||
        getcPtr == NULL) return STR_ERR;
    d = 0;
    e = b->mlen - 2;
    ...
    while ((c = getcPtr(parm)) >= 0) {
        if (d > e) {
            b->slen = d;
            if (Alloc(b, d + 2) != STR_OK) return STR_ERR;
            e = b->mlen - 2;
        }
        b->data[d] = (unsigned char)c;
        d++;
 ...
 */
int GetSa(String b, bNgetc getcPtr, void* parm, char terminator) {
    int c, d, e;
 ...
 */
String GetC(bNgetc getcPtr, void* parm, char terminator) {
    String buff;

    if (0 > GetSa(buff = FromCStr(""), getcPtr, parm, terminator) ||
        0 >= buff->slen) {
        Destroy(buff);
        buff = NULL;
    }
 ...
 */
struct Stream* StreamOpen(bNread readPtr, void* parm) {
    struct Stream* s;

    if (readPtr == NULL) return NULL;
    s = (struct Stream*)StrAlloc(sizeof(struct Stream));
 ...
 */
int StreamBuffLength(struct Stream* s, int sz) {
    int oldSz;
    if (s == NULL || sz < 0) return STR_ERR;
    oldSz = s->maxBuffSz;
    if (sz > 0) s->maxBuffSz = sz;
    return oldSz;
}

int StreamEOF(const struct Stream* s) {
    if (s == NULL || s->readFnPtr == NULL) return STR_ERR;
 ...
 */
int StreamReadLineA(String r, struct Stream* s, char terminator) {
    int i, l, ret, rlo;
    char* b;
    struct _TagString x;

    if (s == NULL || s->buff == NULL || r == NULL || r->mlen <= 0 ||
        r->slen < 0 || r->mlen < r->slen) return STR_ERR;
    l = s->buff->slen;
    if (STR_OK != Alloc(s->buff, s->maxBuffSz + 1)) return STR_ERR;
    b = (char*)s->buff->data;
    ...
    if (i < l) {
        ...
        ret = Concat(r, &x);
        s->buff->slen = l;
        if (STR_OK == ret) Remove(s->buff, 0, i + 1);
        return STR_OK;
    }
 ...
 */
int StreamReadLinesA(String r, struct Stream* s, ConstString term) {
    int i, l, ret, rlo;
    unsigned char* b;
    struct _TagString x;
    struct charField cf;
    ...
    if (s == NULL || s->buff == NULL || r == NULL || term == NULL ||
        ...
        r->mlen < r->slen) return STR_ERR;
    if (term->slen == 1) return StreamReadLineA(r, s, term->data[0]);
    if (term->slen < 1 || buildCharField(&cf, term)) return STR_ERR;
 ...
 */
int StreamReadA(String r, struct Stream* s, int n) {
    int l, ret, orslen;
    char* b;
    struct _TagString x;

    if (s == NULL || s->buff == NULL || r == NULL || r->mlen <= 0
        || r->slen < 0 || r->mlen < r->slen || n <= 0) return STR_ERR;

    if (n > INT_MAX - r->slen) return STR_ERR;
    n += r->slen;
    ...
    do {
        if (l + r->slen >= n) {
            ...
            ret = Concat(r, &x);
            s->buff->slen = l;
            if (STR_OK == ret) Remove(s->buff, 0, x.slen);
            return STR_ERR & -(r->slen == orslen);
        }

        x.slen = l;
        if (STR_OK != Concat(r, &x)) break;
 ...
 *  returned, but will be retained for subsequent read operations.
 */
int StreamReadLine(String r, struct Stream* s, char terminator) {
    if (s == NULL || s->buff == NULL || r == NULL || r->mlen <= 0)
        return STR_ERR;
    if (STR_OK != Alloc(s->buff, s->maxBuffSz + 1)) return STR_ERR;
    r->slen = 0;
    return StreamReadLineA(r, s, terminator);
}
 ...
 *  are not returned, but will be retained for subsequent read operations.
 */
int StreamReadLines(String r, struct Stream* s, ConstString term) {
    if (s == NULL || s->buff == NULL || r == NULL || term == NULL
        || term->data == NULL || r->mlen <= 0) return STR_ERR;
    if (term->slen == 1) return StreamReadLine(r, s, term->data[0]);
    if (term->slen < 1) return STR_ERR;
    if (STR_OK != Alloc(s->buff, s->maxBuffSz + 1)) return STR_ERR;
    r->slen = 0;
    return StreamReadLinesA(r, s, term);
}
 ...
 *  additional characters from the core stream beyond virtual stream pointer.
 */
int StreamRead(String r, struct Stream* s, int n) {
    if (s == NULL || s->buff == NULL || r == NULL || r->mlen <= 0
        || n <= 0) return STR_ERR;
    if (STR_OK != Alloc(s->buff, s->maxBuffSz + 1)) return STR_ERR;
    r->slen = 0;
    return StreamReadA(r, s, n);
}
 ...
 *  stream.
 */
int StreamUnread(struct Stream* s, ConstString b) {
    if (s == NULL || s->buff == NULL) return STR_ERR;
    return Insert(s->buff, 0, b, (unsigned char)'?');
}
 ...
 */
int StreamPeek(String r, const struct Stream* s) {
    if (s == NULL || s->buff == NULL) return STR_ERR;
    return Assign(r, s->buff);
}
 ...
 */
String JoinBulk(const struct StringList* bl, const void* blk, int len) {
    String b;
    unsigned char* p;
    int i, c, v;
...
String Join(const struct StringList* bl, ConstString sep) {
    ...
    return JoinBulk(bl, sep->data, sep->slen);
}
 ...
 *  undefined manner.
 */
int StreamSplitsCb(struct Stream* s, ConstString splitStr,
    ...
    if (cb == NULL || s == NULL || s->readFnPtr == NULL ||
        splitStr == NULL || splitStr->slen < 0) return STR_ERR;

    if (NULL == (buff = FromCStr(""))) return STR_ERR;

    if (splitStr->slen == 0) {
        while (StreamReadA(buff, s, BSSSC_BUFF_LEN) >= 0);
        ...
        for (;;) {
            if (i >= buff->slen) {
                StreamReadA(buff, s, BSSSC_BUFF_LEN);
                if (i >= buff->slen) {
            ...
            if (testInCharField(&chrs, buff->data[i])) {
                ...
                BulkToString(t, buff->data + i + 1, buff->slen - (i + 1));
                if ((ret = StreamUnread(s, &t)) < 0) break;
                buff->slen = i;
                c = buff->data[i];
 ...
 *  undefined manner.
 */
int StreamSplitsStringCb(struct Stream* s, ConstString splitStr,
                         int (*cb) (void* parm, int ofs, ConstString entry), void* parm) {
    String buff;
    int i, p, ret;
    ...
    if (splitStr->slen == 1) return StreamSplitsCb(s, splitStr, cb, parm);

    if (NULL == (buff = FromCStr(""))) return STR_ERR;

    if (splitStr->slen == 0) {
        for (i = 0; StreamReadA(buff, s, BSSSC_BUFF_LEN) >= 0; i++) {
            if ((ret = cb(parm, 0, buff)) < 0) {
                Destroy(buff);
                return ret;
            }
 ...
 */
int SplitCb(ConstString str, unsigned char splitChar, int pos,
            int (*cb) (void* parm, int ofs, int len), void* parm) {
    int i, p, ret;
...
static int bscb(void* parm, int ofs, int len) {
    ...
    if (g->bl->qty >= g->bl->mlen) {
        ...
        while (g->bl->qty >= mlen) {
            if (mlen < g->bl->mlen) return STR_ERR;
            mlen += mlen;
        }

        tbl = (String*)realloc(g->bl->entry,
                               sizeof(String) * mlen);
        if (tbl == NULL) return STR_ERR;
 ...
 *  splitChar.
 */
struct StringList* Split(ConstString str, unsigned char splitChar) {
    ...
    g.b = (String)str;
    g.bl->qty = 0;
    if (SplitCb(str, splitChar, 0, bscb, &g) < 0) {
...
struct StringList* SplitStr(ConstString str, ConstString splitStr) {
    ...
    g.b = (String)str;
    g.bl->qty = 0;
    if (SplitsStrCb(str, splitStr, 0, bscb, &g) < 0) {
        StringListDestroy(g.bl);
        return NULL;
    }
    return g.bl;
 ...
 *  to this end point.
 */
int FormatA(String b, const char* fmt, ...) {
    va_list arglist;
    String buff;
    int n, r;

    if (b == NULL || fmt == NULL || b->data == NULL || b->mlen <= 0
        || b->slen < 0 || b->slen > b->mlen) return STR_ERR;
    ...
    if (NULL == (buff = FromCStrAlloc(n + 2, ""))) {
        n = 1;
        if (NULL == (buff = FromCStrAlloc(n + 2, ""))) return STR_ERR;
    }
 ...
 */
int AssignFormat(String b, const char* fmt, ...) {
    va_list arglist;
    String buff;
    int n, r;
...
String Format(const char* fmt, ...) {
    ...
    if (NULL == (buff = FromCStrAlloc(n + 2, ""))) {
        n = 1;
        if (NULL == (buff = FromCStrAlloc(n + 2, ""))) return NULL;
    }
 ...
 *  to this end point.
 */
int VcFormat(String b, int count, const char* fmt, va_list arg) {
    int n, r, l;

    if (b == NULL || fmt == NULL || count <= 0 || b->data == NULL
        || b->mlen <= 0 || b->slen < 0 || b->slen > b->mlen) return STR_ERR;

    if (count > (n = b->slen + count) + 2) return STR_ERR;
    if (STR_OK != Alloc(b, n + 2)) return STR_ERR;
